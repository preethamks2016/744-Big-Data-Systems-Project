...
def main(batchSize, numWorkers, outputFile, numThreads):
    ...
    hdf5_path = './testCompOpts4.hdf5'
    hf = h5py.File(hdf5_path, 'r')
    dataset = CustomDataset(hf, batchSize)
    data_loader = DataLoader(dataset, 1, shuffle=False, num_workers=2)
    ...
    for idx, (imgs,label) in enumerate(data_loader):
        ...
        # print(imgs.dtype)
        imgs = imgs.squeeze()
        label = label.squeeze().to(torch.long)
